...
DEFAULT_CONFIG = {
    ...
    "guess_lang": [0, "Automatic language detection - Default: False"],
...
class Highlight:
    ...
    def get_lexer(self, src, language, inline, stripnl):
        ...
        if lexer is None:
            if (self.guess_lang is True) or (
                self.guess_lang == "inline" if inline else self.guess_lang == "block"
            ):
                try:
    ...
    def highlight(
        ...
        if pygments and self.use_pygments:
            if p_ver < (2, 12):  # pragma: no cover
                raise RuntimeError(
                    "Pymdownx Highlight requires at least Pygments 2.12+ if enabling Pygments"
                )
            ...
            formatter = html_formatter(
                ...
                linespans=f"{self.line_spans}-{lineno_id}"
                if self.line_spans and not inline
                else "",
                lineanchors=(
                    f"{self.line_anchors}-{lineno_id}"
                    ...
                    if class_names:
                        if m.group(2):
                            classes = ' class="{} {}"'.format(
                                " ".join(class_names), m.group(3).strip()
                            )
                        else:
