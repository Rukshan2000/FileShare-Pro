...
class FancyOListProcessor(BlockProcessor):
    ...
    def run(self, parent, blocks):
        ...
        if (
            sibling is not None
            and sibling.tag in self.SIBLING_TAGS
            and sibling.attrib.get("__fancylist", "") == fancy_type
        ):
        ...
        # This is a new, unique list so create parent with appropriate tag.
            ...
            if self.TAG == "ol":
                ...
                if sibling is not None and sibling.attrib.get(
                    "__fancylist", ""
                ).startswith("force"):
                    ...
                    attrib = {
                        "type": self.OL_TYPES[fancy_type],
                        "__fancylist": fancy_type,
                    }
                    if self.inject_style:
                        attrib[
                            "style"
                        ] = f"list-style-type: {OL_STYLE[attrib['type']]};"
                    if self.inject_class:
                        attrib["class"] = f"fancylists-{OL_STYLE[attrib['type']]}"
                    lst = etree.SubElement(parent, self.TAG, attrib)
            else:
    ...
    def get_fancy_type(self, m, first, fancy_type):
        ...
        if fancy_type.startswith("force"):
            ...
            if value == "#" or (
                (ltype.lower() == "decimal" and value.isdigit())
                or (
                    ltype.lower() == "roman"
                    and self.roman_enabled
                    and value.isalpha()
                    and (len(value) > 2 or value.lower() in "ivxlcdm")
                )
                or (
                    ltype.lower() == "alpha"
                    and self.alpha_enabled
                    and len(value) == 1
                    and value.isalpha()
                )
            ):
                fancy_type = (
                    list_type + fancy_type.split("-", 1)[1] if list_type else list_type
                )
        ...
        if value == "#":
            ...
            if value.islower():
                ...
                if self.alpha_enabled and (
                    not self.roman_enabled
                    or (
                        first
                        and (
                            not in_roman
                            or ((list_type + "roman") != fancy_type and value != "i")
                        )
                    )
                ):
                    list_type += "alpha"
                    ...
                    self.alpha_enabled
                    and not first
                    and ((list_type + "alpha") == fancy_type or not in_roman)
                ):
                    list_type += "alpha"
                else:
                    list_type += "roman"
...
class FancyListBlock(Block):
    ...
    NAME = "fancylists"
    ARGUMENT = True
    OL_TYPE = {"1": "decimal", "a": "alpha", "A": "ALPHA", "i": "roman", "I": "ROMAN"}
...
class FancyListExtension(BlocksExtension):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.config = {
            ...
            "inject_class": [False, "Inject a class indicating the 'list-style-type'"],
        }
