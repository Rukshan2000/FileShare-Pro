...
def update_tag(el, fig_type, fig_num, template, prepend, md):
    ...
    if "id" not in el.attrib:
        el.attrib["id"] = f"__{fig_type}_" + "_".join(
            str(x) for x in fig_num.split(".")
        )
        ...
        for el in doc.iter():
            ...
            if el.tag == "figure":
                ...
                while True:
                    ...
                    if (
                        parent.tag == "figure"
                        and parent.attrib["__figure_type"] == fig_type
                    ):
...
class Caption(Block):
    ...
    def on_end(self, block):
        ...
        if prefix and not self.auto:
            ...
            if self.fig_num:
                update_tag(
                    block, self.NAME, self.fig_num, prefix, self.prepend, self.md
                )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.config = {
            "types": [
                [
                    "caption",
                    {"name": "figure-caption", "prefix": "Figure {}."},
                    {"name": "table-caption", "prefix": "Table {}."},
                ],
            ...
            "auto_level": [0, "Depth of children to add prefixes to - Default: 0"],
            "auto": [
                True,
    ...
    def extendMarkdownBlocks(self, md, block_mgr):
        ...
        for obj in config["types"]:
            ...
            block_mgr.register(
                type(
                    subclass,
                    (Caption,),
                    {"OPTIONS": {}, "NAME": name, "PREFIX": prefix, "CLASSES": classes},
                ),
                {
        ...
        if config["auto"]:
            md.treeprocessors.register(
                CaptionTreeprocessor(md, types, config), "caption-auto", 4
            )
