...
RE_LINENUMS = re.compile(
    r"(?P<linestart>[\d]+)(?:[ \t]+(?P<linestep>[\d]+))?(?:[ \t]+(?P<linespecial>[\d]+))?"
)
...
def fence_code_format(source, language, class_name, options, md, **kwargs):
    ...
    return "<pre{}{}{}><code>{}</code></pre>".format(
        id_value, classes, attrs, _escape(source)
    )
    ...
    for k, v in inputs.items():
        ...
        if use_pygments:
            if k.startswith("data-"):
                attrs[k] = v
                continue
            for opt, validator in (
                ("hl_lines", RE_HL_LINES),
                ("linenums", RE_LINENUMS),
                ("title", None),
            ):
                if k == opt:
                    if v is not True and (
                        validator is None or validator.match(v) is not None
                    ):
                        options[k] = v
                        matched = True
                        break
...
def _formatter(
    src="", language="", options=None, md=None, class_name="", _fmt=None, **kwargs
):
    """Formatter wrapper."""
        ...
        self.superfences.insert(
            0,
            {
                ...
                "validator": functools.partial(
                    _validator, validator=highlight_validator
                ),
            },
        )
        ...
        for custom in custom_fences:
            ...
            if name is not None and class_name is not None:
                self.extend_super_fences(
                    name,
                    functools.partial(
                        _formatter, class_name=class_name, _fmt=fence_format
                    ),
    ...
    def parse_options(self, m):
        ...
        for entry in reversed(self.extension.superfences):
            if entry["test"](self.lang):
                ...
                validator = entry.get(
                    "validator",
                    functools.partial(_validator, validator=default_validator),
                )
                try:
...
class SuperFencesRawBlockPreprocessor(SuperFencesBlockPreprocessor):
    ...
    def reassemble(self, lines):
        ...
        while len(self.stack):
            ...
            lines = (
                lines[:start]
                + [fenced.replace(md_util.STX, SOH, 1)[:-1] + EOT]
                + lines[end:]
            )
        return lines
