...
def get_deps(projects_file_url: str, config_file_path: str | None = None) -> None:
    ...
    if all(
        c not in cfg for c in ("site_name", "theme", "plugins", "markdown_extensions")
    ):
        log.warning(
            "The passed config file doesn't seem to be a mkdocs.yml config file"
        )
    else:
    ...
    wanted_plugins = (
        (
            PluginKind("mkdocs_theme", "mkdocs.themes"),
            ...
            for entry_name in available:
                ...
                if entry_name in wanted:
                    if "pypi_id" in project:
                        ...
                        install_name = "git+https://github.com/{github_id}".format_map(
                            project
                        )
                    else:
                        log.error(
                    ...
                    for extra_key, extra_pkgs in project.get(
                        "extra_dependencies", {}
                    ).items():
